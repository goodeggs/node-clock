...
(function() {
  var clock, tz, tzWithId, tzids, validTzids, zoneData, _,
    __slice = [].slice;
  ...
  tzids = _(zoneData).chain().map(function(data) {
    return _(data.zones).keys();
  }).flatten().value().concat(['UTC']);

  validTzids = _(tzids).groupBy(function(tzid) {
    return tzid;
  });
